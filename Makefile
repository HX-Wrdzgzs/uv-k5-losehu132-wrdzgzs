...
# Detect msys2/cygwin environment (unix-like shell on Windows)
# Override Windows settings for msys2 where 'del' is not available
ifneq ($(MSYSTEM),)
    RM = rm -f
    FixPath = $1
    WHERE = which
...
CFLAGS += -DCUSTOM_TAIL_EMBEDDED=$(CUSTOM_TAIL_EMBEDDED)
...
clean:
	@$(RM) $(TARGET).bin
	@$(RM) $(PACKED_FILE_SUFFIX).bin
	@$(RM) $(TARGET)

ifeq ($(OS), Windows_NT) # Windows 系统
ifneq ($(MSYSTEM),)
	chmod +x del_linux.sh 2>/dev/null; sh ./del_linux.sh
else
	@call del_win.bat
endif
