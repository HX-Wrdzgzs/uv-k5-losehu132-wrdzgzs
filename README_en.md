...
## Latest release: LOSEHU132-bin-20260820-final3

The GitHub Release contains two clearly separated firmware variants:

| Variant | Tail behavior | Packed image |
| --- | --- | --- |
| `public` | No private tail; the menu entry remains and uses the normal Roger tone | [`...-public.packed.bin`](./release/LOSEHU132-bin-20260820-final3-public/LOSEHU132-bin-20260820-final3-public.packed.bin) |
| `tail` | Personal `1.wav` single-tone/envelope approximation with the unstable opening removed | [`...-tail.packed.bin`](./release/LOSEHU132-bin-20260820-final3-tail/LOSEHU132-bin-20260820-final3-tail.packed.bin) |

Use the `public` image for a no-private-tail installation and the `tail` image for the personal-tail installation. The tail image removes about 180 ms of low-level unstable pitch tracking from the opening to avoid a tremolo-like start. The original `1.wav` and legacy external `tails.bin` are not uploaded; the personal tail is compiled into the `tail` firmware. `repeaters.bin` is a separate K5DB v3 database and must be written through the database tool, not the firmware flasher. See the [Chinese release instructions](./README.md) for COM4 and database details.
