...
#ifdef ENABLE_TIMER
    if (UI_MENU_GetCurrentMenuId() == MENU_RTC_SET) {
        // Enter 6 digits for HHMMSS
        if (gInputBoxIndex < 6) {
            gRequestDisplayScreen = DISPLAY_MENU;
            return;
        }
        gInputBoxIndex = 0;
        uint8_t hh = gInputBox[0] * 10 + gInputBox[1];
        uint8_t mm = gInputBox[2] * 10 + gInputBox[3];
        uint8_t ss = gInputBox[4] * 10 + gInputBox[5];
