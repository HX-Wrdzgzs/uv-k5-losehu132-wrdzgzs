...
void UI_DisplayStatus() {
    ...
    {
        char time_str[8];
        time_str[0] = '0' + (time[3] / 10);
        time_str[1] = '0' + (time[3] % 10);
        time_str[2] = ':';
        time_str[3] = '0' + (time[4] / 10);
        time_str[4] = '0' + (time[4] % 10);
        time_str[5] = '\0';
        unsigned int tw = 6 * 6; // 6 chars * 6px small font width
        if (x + tw < LCD_WIDTH - 46) { // enough space?
            UI_PrintStringSmallBuffer(time_str, line + x);
            x += tw;
            x1 = MAX(x1, x);
        }
    }
